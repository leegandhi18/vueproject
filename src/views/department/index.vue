    ...
    <div>
      <b-table small hover striped :items="departmentList" :fields="fields">
        ...
        <template #cell(updatedBtn)="row">
          <b-button variant="success" @click="onClickEdit(row.item.id)">수정</b-button>
        </template>
        <template #cell(deletedBtn)="row">
          <b-button variant="danger" @click="onClickDelete(row.item.id)">삭제</b-button>
        </template>
      </b-table>
    </div>
